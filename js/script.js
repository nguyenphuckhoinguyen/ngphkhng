...
const removebut = new MutationObserver((mutationsList) => {
  for (let mutation of mutationsList) {
    if (mutation.type === 'childList') {
      ...
      const buyItems = document.querySelectorAll('.remove-button')
...
function displayCart() {
  ...
  cart.forEach((item, index) => {
    ...
    cartItemElement.innerHTML = `<img src=""><span>${item.name} - Giá: $${item.price} - Số lượng: ${item.quantity}  
                                <button class="remove-button" data-index="${index}">Xóa</button></span>`
